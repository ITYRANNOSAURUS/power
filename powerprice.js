...
function getPowerPrice() {
    ...
    if (price === undefined) {
        document.getElementById('powerprice-result').innerText = year + "년도의 데이터는 아직 준비되지 않았습니다.";
    } else {
        let baseYear = 2022;
        let basePrice = powerPriceData[baseYear];
        let increaseRate = ((price - basePrice) / basePrice * 100).toFixed(2);
        document.getElementById('powerprice-result').innerText = year + "년의 예상 전기 단가는 " + price +
